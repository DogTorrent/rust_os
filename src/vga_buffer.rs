...
/// Combination of a foreground(the first 4 bits) and a background(the last 4 bits) color.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)]
struct ColorCode(u8);
    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                ...
                // For unprintable bytes(not part of printable ASCII range), print a ■ character, which has the hex code 0xfe on the VGA hardware
...
impl fmt::Write for Writer {
    /// Implement the core::fmt::Write trait by implement the only required method 'write_str'.
